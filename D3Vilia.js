/**
 * The D3Vilia edition game system for Foundry Virtual Tabletop
 * Author: LoopeeDK, Rughalt
 * Software License: GNU GPLv3
 */

// Import Modules
import { D3Vilia } from "./module/config.js";
...
import D3ViliaLayer from "./module/layer.js";
...
Hooks.once("init", async function() {
  console.log(`D3Vilia | Initializing D3Vilia 1 System`);

  // Create a D3Vilia namespace within the game global
  game.D3Vilia = {
    ActorPF,
    DicePF,
    ItemPF,
  ...
  Actors.registerSheet("D3Vilia", ActorSheetPFCharacter, { types: ["character"], makeDefault: true });
  Actors.registerSheet("D3Vilia", ActorSheetPFNPC, { types: ["npc"], makeDefault: true });
  Actors.registerSheet("D3Vilia", ActorSheetPFNPCLite, { types: ["npc"], makeDefault: false });
  Actors.registerSheet("D3Vilia", ActorSheetPFNPCLoot, { types: ["npc"], makeDefault: false });
  Actors.registerSheet("D3Vilia", ActorSheetPFNPCMonster, { types: ["npc"], makeDefault: false });
  Actors.registerSheet("D3Vilia", ActorSheetTrap, { types: ["trap"], makeDefault: true });
  Items.unregisterSheet("core", ItemSheet);
  Items.registerSheet("D3Vilia", ItemSheetPF, { types: ["class", "feat", "spell", "consumable","equipment", "loot", "weapon", "buff", "attack", "race", "enhancement","damage-type","material","full-attack"], makeDefault: true });
...
Hooks.once("setup", function() {
  ...
  for ( let o of toLocalize ) {
    try {
      CONFIG.D3Vilia[o] = doLocalize(CONFIG.D3Vilia[o]);
    } catch (e) {
      //ignore
    }
  }
...
Hooks.once("ready", async function() {
  ...
  $('body').toggleClass('hide-special-action', !game.settings.get("D3Vilia", "allowPlayersApplyActions"));
  ...
  let PREVIOUS_MIGRATION_VERSION = game.settings.get("D3Vilia", "systemMigrationVersion");
  ...
  let isDemo = game.settings.get("D3Vilia", "demoWorld")
  if (isDemo) {
  ...
  console.log("D3Vilia | Cache is ", CACHE)
...
Hooks.on("canvasInit", function() {
  ...
  canvas.grid.diagonalRule = game.settings.get("D3Vilia", "diagonalMovement");
...
Hooks.on("renderChatMessage", (app, html, data) => {
  ...
  if (game.settings.get("D3Vilia", "autoCollapseItemCards")) html.find(".card-content.item").hide();
});
...
Hooks.on("renderTokenConfig", async (app, html) => {
  ...
  let newHTML = await renderTemplate("systems/D3Vilia/templates/internal/token-light-info.html", {
    object: duplicate(app.object.data.actorLink ? game.actors.get(app.object.data.actorId).data : app.object.data.actorData),
    globalDisable: game.settings.get("D3Vilia", "globalDisableTokenLight")
  });
  html.find('.tab[data-tab="vision"] > *:nth-child(5)').after(newHTML);
  let newHTML2 = await renderTemplate("systems/D3Vilia/templates/internal/token-config.html", {
...
Hooks.on("createMeasuredTemplate", (scene, _template, data, user) => {
  game.D3Vilia.createdMeasureTemplates.add(_template._id)
});
...
async function createItemMacro(item, slot) {
  ...
  const command = `game.D3Vilia.rollItemMacro("${item.name}", {\n` +
  `  itemId: "${item._id}",\n` +
  `  itemType: "${item.type}",\n` +
...
Hooks.on("getSceneControlButtons", (controls) => {
  ...
  controls.push({
    name: "d3vilia-gm-tools",
    title: "D3Vilia.GMTools",
    icon: "fas fa-dungeon",
    layer: "D3ViliaLayer",
    tools: [
      {
        name: "d3vilia-gm-tools-encounter-generator",
        title: "D3Vilia.EncounterGenerator",
        icon: "fas fa-dragon",
